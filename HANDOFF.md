...
| 3 | 獨立小型壓力測試腳本,量出單一 Redis 節點面對瞬間爆量的限流 CAS 檢查實際能撐多少併發(在 OCI 上測) | 🔧 **進行中**(因 §2.8 從階段 1 之後提前)。3-1 已跑(2026-09-23):50→1600 併發 ops/s 從 1700 起每翻倍減半,拐點在 50 以下未量到,單一 key 交不出 3000/s(報告 §14.4)。**下一步 3-2**:`sweep.sh 1 2 5 10 20 30 50`(見 `load-test/ratelimit-bench/README.md`) |
