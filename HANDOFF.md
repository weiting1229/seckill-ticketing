...
> 最後更新:2026-09-23。**本文件只放需要交接給下一個 session 的待辦事項**,不放已結案的
> 排查過程或原始數據——那些在
...
壓測計畫的問題已回答完。剩下的都是**會改正式站行為**的決策,尚未動工:

- **要不要實作 sharded counter(建議 4 片)**。會改 `RateLimiterService` 的限流行為,需寫 ADR。
  已知限制與替代方向(單一原子 Lua token bucket,未評估)見計畫 §5.3。
- 階段 2(下游承載)要不要做。
