...
**壓測後約 20 分鐘的二次觀測(順帶驗證逾時取消)**:309 筆訂單全部轉為 `EXPIRED`,
DB 庫存回到 1000、Redis 庫存回到 1000、`stock_logs` 淨變動回到 **0**(−309 之後又 +309)。
**逾時取消與庫存歸還在 309 筆規模下正確**。這也再次印證 §11.4 的教訓:對帳必須趁 15 分鐘內做,
上表的對帳數字取自過期前;若拖過 15 分鐘再查,看到的會是「賣出 0 張」的假象。
...
### 14.4 輪 3-1:單一 Redis 節點的限流 CAS 上限,第一輪(2026-09-23)—— 拐點落在 50 以下

計畫 §4(階段 3,因 §2.8 提前)。工具:`load-test/ratelimit-bench/`,在 OCI 主機的 docker 網路內
對**拋棄式** `seckill-redis-bench`(規格對齊正式 Redis)打同一把 key `seckill:rl:global`,
capacity 3000、每階暖身 5s + 計時 30s、閉環(併發數 = 在途請求數)。正式站 Redis 未受影響。

| 併發 | ops/s | throttled | errors | p50 | p90 | p99 | p99.9 | max | ops/s × 併發 | 平均延遲(Little) |
|---|---|---|---|---|---|---|---|---|---|---|
| 50 | **1700** | 0 | 0 | 18ms | 81ms | 181ms | 283ms | 0.53s | 85,000 | 29ms |
| 100 | 893 | 0 | 0 | 72ms | 307ms | 673ms | >1s | 1.5s | 89,300 | 112ms |
| 200 | 459 | 0 | 0 | 296ms | >1s | >1s | >1s | 6.5s | 91,800 | 0.44s |
| 400 | 226 | 0 | 0 | >1s | >1s | >1s | >1s | 22s | 90,400 | 1.8s |
| 800 | 112 | 0 | 0 | >1s | >1s | >1s | >1s | 35s | 89,600 | 7.1s |
| 1600 | 53 | 0 | 0 | >1s | >1s | >1s | >1s | 35s | 84,800 | 30s(=計時窗,已截斷) |

#### 讀到的事

1. **沒有「上升後持平」的一段,從第一階就在下降**:併發每翻倍,ops/s 就減半。
   拐點(吞吐最高的那一階)**在 50 以下**,這一輪沒有量到。本機 smoke test 的 20 併發是 2801 ops/s,
   方向一致但不可外推。
2. **`ops/s × 併發` 幾乎是常數(85k–92k)**。這正是 CAS 重試風暴的形狀:N 個競爭者同時讀到
   同一個版本,只有一個 CAS 成功,其餘重試 —— 每成功一次約要付出 N 次嘗試,有效吞吐 ≈ 固定的
   「嘗試容量」÷ N。**這是推論,尚未驗證**:缺 Redis 指令計數,也還分不出那個固定容量是
   Redis 單執行緒的上限,還是 client 端單一連線 netty event loop 的上限(見 3-2)。
3. **throttled 全為 0**:最高吞吐 1700/s 本身就低於 capacity 3000/s,桶從未被取空。
   也就是說,**在 50 併發以上,這個 CAS 限流器自己就交不出 3000/s,`global-capacity=3000` 這個門檻
   在單一 key 上達不到**,下游永遠等不到限流器放行那麼多流量。
4. 與正式站的對應:正式站的「併發」= 同時在做 global 檢查的請求數 ≈ 到達率 × 單次檢查耗時。
   輪 1-2 只有約 65 次/秒、global p99 3.5ms,在途數遠小於 1,所以當時看不到排隊 —— 與本表不矛盾。

#### 下一輪(3-2)要補的

- **低併發段**:1、2、5、10、20、30、50,找真正的吞吐峰值。
- **每階單獨跑並抓 `INFO commandstats` 與兩端容器 CPU**(`load-test/ratelimit-bench/sweep.sh`),
  驗證第 2 點的推論,並分辨瓶頸在 Redis 還是 client 端。

### 14.5 輪 3-2:低併發段 + Redis 指令計數(2026-09-23)—— 重試風暴坐實,單 key 在途 10–20 是拐點

`sweep.sh 1 2 5 10 20 30 50`,每階單獨跑(暖身 5s + 計時 30s),前後 `CONFIG RESETSTAT` / `INFO commandstats`,
計時窗中段 `docker stats`。capacity 3000,其餘同 §14.4。

| 併發 | ops/s | 放行/s | 拒絕/s | p50 | p99 | CAS 嘗試 ÷ 成功寫入 | Redis CPU | bench CPU |
|---|---|---|---|---|---|---|---|---|
| 1 | 10,051 | 3,000 | 7,051 | 0.08ms | 0.26ms | **1.00** | 28% | 55% |
| 2 | 16,879 | 3,000 | 13,879 | 0.07ms | 0.63ms | **1.99** | 38% | 89% |
| 5 | **18,148** | 3,000 | 15,148 | 0.12ms | 1.07ms | **4.69** | 52% | 116% |
| 10 | 8,207 | 3,001 | 5,207 | 0.70ms | 7.3ms | **8.70** | 61% | 122% |
| 20 | 3,062 | 3,062 | **0** | 3.7ms | 40ms | **11.35** | 68% | 109% |
| 30 | 2,505 | 2,505 | 0 | 6.6ms | 75ms | **16.07** | 69% | 110% |
| 50 | 1,669 | 1,669 | 0 | 17ms | 184ms | **25.6** | 67% | 111% |

「CAS 嘗試 ÷ 成功寫入」= `cmdstat_eval.calls` ÷ `cmdstat_psetex.calls`。指令對應是**從計數推定**,不是讀 Bucket4j 原始碼確認:
EVAL 是一次 CAS 嘗試;PSETEX 在 CAS 腳本內、只有比對成功才執行(它的次數在每一階都 ≈ 放行數 × 35/30,
即含暖身的放行數);GET 在併發 ≥20 時剛好是 EVAL 的 2 倍(client 讀一次 + 腳本內比對讀一次)。
被拒絕的請求只有 GET、沒有 EVAL(併發 1 時 EVAL ≈ PSETEX,21 萬次拒絕沒有產生任何 CAS)。

#### 讀到的事

1. **重試風暴坐實**:每成功一次要付出的 CAS 嘗試數隨併發幾乎線性成長(1 → 2 → 4.7 → 8.7 → … → 25.6)。
   而且**併發 2–5 時 Redis 與 client 都還沒有飽和,比值就已經是 N**,所以這是樂觀 CAS 本身的性質,
   不是某一端資源不夠才出現的現象。資源上限只決定曲線的絕對高度。
2. **拒絕很便宜、放行很貴**:拒絕只有一次 GET,放行要走 CAS。所以併發 ≤10、桶會被取空時,
   總吞吐可以到 1.8 萬/s(大部分是拒絕);併發 20 起**桶再也取不空**(拒絕 = 0),每個請求都走 CAS,
   吞吐直接掉到放行上限附近。
3. **拐點在在途 10–20 之間**:併發 10 時仍穩穩放行 3000/s、p99 7ms;併發 20 放行剛好 3062/s 但 p99 40ms,
   已經在邊緣;30 起放行掉到 capacity 以下(2505 → 1669),限流器**自己**變成比設定值更緊的瓶頸。
   用 Little 定律換算,併發 20 的平均延遲約 6.5ms —— 也就是**到達率 3000/s 時只要單次檢查超過約 6.5ms,
   在途數就會跨過拐點**,之後延遲變長 → 在途更多 → 重試更多,是正回饋。
4. **瓶頸在哪一端仍未定**:Redis 從併發 20 起卡在單核 67–69% 持平,沒有打滿;bench 容器卡在 109–122% 持平。
   比較像 client 端某條執行緒(推測是 Lettuce 單一連線的 I/O 執行緒)先滿,但 `docker stats` 只有容器總量,
   **未坐實**。正式站 backend 同樣是 Bucket4j 共用單一連線,所以這個上限對正式站仍有代表性。
5. 併發 50 與 §14.4 同階的數字吻合(1669 vs 1700 ops/s、p99 184 vs 181ms),兩輪可以銜接。

#### 對階段 4 的影響

計畫 §5.1 的公式把「單節點安全上限」當成一個**速率**。這一輪顯示限制其實是**同一把 key 的在途數**:
同一個 Redis 節點上把 key 拆成 S 片,每片的在途數就變成 1/S,重試次數跟著降,不需要多個節點。
所以分片數比較接近 `ceil(預期在途數 ÷ 單 key 安全在途數)`,而不是速率相除。§5 要改寫,見計畫 §4.3。

另外,放行 3000/s 是被 capacity 卡住的,**單 key 真正的 CAS 寫入上限還沒量到**(併發 ≤10 時桶都被取空)。

### 14.6 輪 3-3:不被 capacity 卡住的純 CAS 寫入上限(2026-09-23)—— 單 key 最多約 6,200 次/秒,且在併發 1

`BENCH_CAPACITY=100000000 sweep.sh 1 2 5 10 20`:桶永遠取不空,每個請求都走 CAS。每階先 `DEL` key。
新增 bench JVM 各執行緒 CPU(`ps -L`,**是執行緒存活期間的平均,不是瞬時值**,約含 1–2 秒啟動時間)。

| 併發 | 放行/s | p50 | p99 | CAS 嘗試 ÷ 成功寫入 | Redis CPU | bench CPU | Lettuce I/O 執行緒 |
|---|---|---|---|---|---|---|---|
| 1 | **6,240** | 0.15ms | 0.30ms | 1.00 | 32% | 56% | 32% |
| 2 | 5,825 | 0.16ms | 5.4ms | 2.00 | 41% | 85% | 53% |
| 5 | 5,364 | 0.46ms | 6.5ms | 3.95 | 57% | 125% | 64% |
| 10 | 4,680 | 1.1ms | 14ms | 6.49 | 65% | 120% | 67% |
| 20 | 3,383 | 3.4ms | 36ms | 11.2 | 69% | 115% | 69% |

#### 讀到的事

1. **單 key 的 CAS 放行上限在併發 1 就達到,約 6,200/s**,之後併發越高越低。在途 1 時每次 GET + EVAL
   約 160µs,也就是兩次往返。加併發不但沒有攤掉往返,反而因為同一版本只能有一個 CAS 成功而全部變成重試。
2. **兩端資源都沒有滿**:Redis 單核最高 69%,Lettuce I/O 執行緒最高 69%(存活平均,扣掉啟動時間約 75%)。
   §14.5 猜的「client 端單一執行緒先滿」**不成立**。比較能解釋數字的是:上限由**往返延遲 + 單版本
   只能一個成功**決定,不是由 CPU 決定。這仍是推論,但 CPU 證據已排除「單純某一端被打滿」。
3. 與 §14.5 銜接:併發 20 時 3-2 放行 3,062/s、3-3 為 3,383/s,差距來自 3-2 的桶偶爾被取空。
4. **對 `global-capacity=3000` 的意義**:單一 key 要放行 3,000/s,在途數必須壓在 20 以下,
   且此時 p99 已經 36ms。沒有餘裕 —— 在途數再多一點,放行量就掉到設定值以下,延遲同時惡化。

#### 階段 3 結論

**一把 key 用 Bucket4j 樂觀 CAS 的安全範圍約是在途 ≤10(放行上限約 4,700/s、p99 ≤14ms);
絕對上限約 6,200/s(在途 1)。** 瓶頸不在 Redis 的 CPU,所以換更大的 Redis 節點不會改善,
要降低同一把 key 的在途數(拆 key)或不再用樂觀 CAS。

### 14.7 輪 5-1、5-2:原子 Lua token bucket(2026-09-25)—— 一把 key 撐到 8.5 萬次/秒,沒有拐點

計畫 §6。`ratelimit-bench` 的 `BENCH_MODE=lua`([`token_bucket.lua`](../load-test/ratelimit-bench/src/main/resources/token_bucket.lua)),
同一台 OCI 主機、同一個拋棄式 Redis 規格、同一支 `sweep.sh`(暖身 5s + 計時 30s,每階先 `DEL` key),
與 §14.5、§14.6 直接對照。bench JVM 執行緒 CPU 同 §14.6,是存活期間平均。

#### 5-1:capacity 3000(對照 3-2)

| 併發 | 總檢查/s | 放行/s | p50 | p99 | p99.9 | Redis CPU | Lettuce I/O 執行緒 |
|---|---|---|---|---|---|---|---|
| 1 | 9,759 | **3,000** | 0.10ms | 0.20ms | 0.59ms | 42% | 28% |
| 2 | 20,005 | **3,000** | 0.09ms | 0.22ms | 0.71ms | 69% | 47% |
| 5 | 34,617 | **3,000** | 0.14ms | 0.31ms | 1.3ms | 79% | 55% |
| 10 | 50,832 | **3,000** | 0.19ms | 0.47ms | 2.4ms | 86% | 60% |
| 20 | 62,589 | **3,000** | 0.29ms | 0.78ms | 4.2ms | 90% | 61% |
| 50 | 73,094 | **3,000** | 0.62ms | 1.9ms | 6.0ms | 94% | 59% |
| 100 | 80,364 | **3,000** | 1.2ms | 3.0ms | 7.8ms | 95% | 62% |
| 200 | 84,713 | **3,000** | 2.2ms | 5.0ms | 11ms | 94% | 61% |

每階計時窗放行 89,999–90,002 次(= 3000/s × 30s),errors 全 0。

#### 5-2:capacity 1 億、桶取不空(對照 3-3)

| 併發 | Lua 放行/s | CAS 放行/s(3-3) | Lua p99 | CAS p99(3-3) | Redis CPU |
|---|---|---|---|---|---|
| 1 | 10,187 | 6,240 | 0.20ms | 0.30ms | 42% |
| 5 | 34,510 | 5,364 | 0.31ms | 6.5ms | 80% |
| 20 | 63,474 | 3,383 | 0.72ms | 36ms | 90% |
| 50 | 73,964 | — | 1.7ms | — | 86% |
| 100 | 81,947 | — | 2.7ms | — | 94% |
| 200 | 84,581 | — | 5.0ms | — | 92% |
...
兩輪每一階 `cmdstat_evalsha.calls` = `cmdstat_hset.calls`(腳本內每次恰好一次寫入),
且 ÷(ops × 35/30)= 0.96–0.98(略小於 1 是暖身期 JIT 未熱、速率較低)。**沒有任何重送或重試。**
Redis 端每次腳本執行 `usec_per_call` 從併發 1 的 16µs 降到併發 200 的 8.2µs(批次效應)。

#### 讀到的事

1. **正確性成立**:放行量在每一階都精準等於 capacity,不多放,也不因競爭掉到設定值以下(3-2 在併發 30 起就掉了)。
2. **沒有拐點**:從併發 1 到 200,吞吐一路上升、p99 平緩增加,量測範圍內 p99 最高 5ms,
   沒有碰到階段 3 的 14ms 安全線。延遲增加是 Redis 單執行緒排隊(Little 定律:84,713/s × 平均約 2.3ms ≈ 200),
   不是重試。
3. **上限移到 Redis 單執行緒**:約 8.5 萬次/秒時 Redis CPU 到 92–95%、吞吐趨平。這是一般意義上的
   「Redis 忙不過來」,不再是 CAS 的「同一版本只能一個成功」。是 CAS 絕對上限(6,240/s)的約 13.6 倍,
   是目標 3000/s 的約 28 倍。
4. **放行與拒絕一樣貴**:5-1 與 5-2 同併發的數字幾乎相同 —— 被拒絕的請求一樣跑完整個腳本並寫回。
   與 CAS 相反(CAS 拒絕只要一次 GET)。所以要拿來跟到達率比的是「總檢查/s」上限 8.5 萬,不是放行數。

#### 階段 5 結論

**一把 key 用原子 Lua token bucket,在 OCI 上的安全在途數 ≥ 200(量測上限,未碰到 p99 14ms),
總檢查上限約 8.5 萬次/秒。** 依計畫 §6.3 判定為「一把 key 就撐得住,不需要分片」。

已知限制:
- 拋棄式 Redis 上**只有這一把 key 的流量**。正式 Redis 同時要跑扣庫存、token 校驗等腳本,以及另外三層限流;
  8.5 萬是這個腳本獨佔 Redis 時的上限,正式站可分到的份額較低。
- bench 是單一 JVM、單一連線。正式站多個 backend 實例各自一條連線時,Redis 端的批次效應會不同,未量。

### 14.8 輪 6-1、6-2:M9 正式限流腳本(三把 key、全有或全無)(2026-09-25)—— 實際情境 9 萬次/秒,最壞情境 3.3 萬次/秒

ADR 0010 §9。`BENCH_MODE=lua-purchase`:jar 直接打包 `backend/src/main/resources/lua/ratelimit_token_bucket.lua`,
每次檢查帶全域 + 每次新的 IP + 每次新的用戶三把 key(單 IP 10、單用戶 2,對齊正式站預設)。其餘同 §14.7。

#### 6-1:全域 capacity 3000(實際情境:放行 3000/s,其餘被全域層擋下)

| 併發 | 總檢查/s | 放行/s | p50 | p99 | p99.9 | Redis CPU |
|---|---|---|---|---|---|---|
| 1 | 9,675 | **3,000** | 0.10ms | 0.21ms | 1.1ms | 42% |
| 5 | 33,321 | **3,000** | 0.14ms | 0.34ms | 1.8ms | 61% |
| 20 | 65,866 | **3,000** | 0.28ms | 0.80ms | 3.9ms | 83% |
| 50 | 84,204 | **3,000** | 0.50ms | 1.7ms | 5.2ms | 75% |
| 100 | 90,709 | **3,000** | 1.2ms | 3.0ms | 6.5ms | 89% |
| 200 | 90,418 | **3,000** | 2.0ms | 5.3ms | 9.9ms | 91% |

每階 `hset` ≈ 32.4 萬 = 含暖身的放行數(3000 × 35 + 初始滿桶 3000)× 3 把 key:**只有放行才寫,被擋下的請求一次寫入都沒有**。
被全域層擋下的請求只執行一次 `HMGET` 就返回。

#### 6-2:全域 capacity 1 億(最壞情境:每個請求都放行,每次讀寫三把 key、新建兩把)

| 併發 | 放行/s | p50 | p99 | p99.9 | Redis CPU |
|---|---|---|---|---|---|
| 1 | 7,921 | 0.12ms | 0.24ms | 1.9ms | 49% |
| 5 | 23,861 | 0.18ms | 0.48ms | 7.4ms | 118% |
| 20 | 30,377 | 0.55ms | 2.1ms | 26ms | 94% |
| 50 | 32,094 | 1.3ms | 7.5ms | 30ms | 97% |
| 100 | 33,032 | 2.6ms | **17ms** | 32ms | 95% |
| 200 | 33,746 | 4.9ms | **31ms** | 39ms | 124% |

Redis 端每次腳本 19–29µs(單 key 版 8µs)。Redis CPU 超過 100% 是主執行緒之外的背景執行緒(AOF 寫檔、大量短 TTL key 過期)。

#### 讀到的事

1. **正確性成立**:6-1 每階放行 90,000–90,001 次 / 30s。
2. **實際情境比單 key 版還快**(9 萬 vs 8.5 萬次/秒,§14.7):被擋下的請求不寫回,在第一個不足的層就返回。
   搶票瞬間絕大多數請求是被擋下的那一種,所以這才是搶票時的形狀。
3. **最壞情境的上限約 3.3 萬次/秒**,安全在途數(p99 ≤ 14ms)落在 50–100 之間。但放行數受全域 capacity 3000 封頂,
   「每個請求都放行且都寫三把 key」在正式設定下最多就是 3000/s,這張表只用來知道 capacity 往上調時的天花板:
   capacity 調到約 3 萬以上才會逼近。

#### M9 結論

**M9 正式腳本在 OCI 上:實際情境約 9 萬次檢查/秒(p99 5.3ms @ 在途 200),放行精準;最壞情境約 3.3 萬次/秒。**
目標 3000/s 的餘裕在兩種情境下都是 10 倍以上。已知限制同 §14.7(拋棄式 Redis 只有限流流量)。
