...
> 建立:2026-09-15|狀態:**階段 1、3、4 已結案(2026-09-23)**。階段 1 由階段 3 替代結案(§2.9);
> 階段 4 算出最少 2 片、建議 4 片(§5.3)。階段 2 未開始,因目標流量改定為 3000/s 已非必要(§5.2)。
> **階段 5(Lua token bucket 評估,§6)已完成**(2026-09-25):一把 key 就撐得住,不需要分片(§6.5)
>
...
| 1-2 | 2026-09-20 | 2000 VU,首次用集中預登入:**量測窗內零登入、CPU 峰值 42%**(預登入本身 90%,冷卻後回 7%);purchase p99 13.7ms、global p99 3.5ms;成交 309 / 429 共 1651 / 5xx 0;對帳一致 | ⚠️ **本輪無效**:對 global key 的壓力只有約 65 次/秒,是門檻 3000/s 的 2%,§2.6 判定表不適用。見 §2.8 |

### 2.9 ✅ 結案:由階段 3 替代(2026-09-23 使用者決定)

階段 1 想問的「global key 在高併發下會不會排隊」,已由階段 3 在 OCI 硬體上直接量到機制
(樂觀 CAS 重試風暴,§4.3),不再改寫情境腳本去產生 3000/s 的 HTTP 流量。
**已知限制**:沒有走完整 HTTP 路徑驗證,所以「秒級 HTTP p99 貼著 global p99」這個端到端現象
在 OCI 上仍未親眼看到;階段 3 的結論是元件層級的。

### 2.8 ⚠️ 階段 1 的真正卡點:現行流量產生方式壓不到門檻量級(2026-09-20 發現)

1-2 跑完才看清楚的結構性問題,原計畫沒有預期到:

**情境 A 每個 VU 只搶一次就 `settled`。** 2000 個 VU 在 30 秒 ramp 內各打一次 purchase,
對 `seckill:rl:global` 的壓力就只有約 **66 次/秒**;ramp 一結束全部 VU 定局,hold 的 120 秒
整段是 idle sleep,沒有任何流量。要驗證的門檻是 `global-capacity=3000`/s ——**實際只壓到 2%**。

所以 §2.6 的判定表在 1-2 不適用:它的第二列(兩個 p99 都毫秒級 → 未重現)前提是有把 global key
壓到會排隊的量。再多跑幾輪 2000 VU 只會重複得到同一組毫秒級數字。

要讓階段 1 能判定,必須先決定**怎麼產生接近 3000/s 的 global key 流量**。已知的限制:

- **單機 k6 是單一 IP,IP 層 10/s**。不論怎麼加壓,能通過 IP 層的永遠只有 10/s;但 global 層在
  IP 層之前,所以「被 429 擋掉的請求也算數」——衝 global 流量不需要動 IP 層閾值。
- **必須改成會重複送出請求的型態**(情境 B 那種 constant-vus + 迴圈),而不是 one-shot。
  粗估:單次 round trip 約 100ms,要撐住 3000/s 需要約 300 個持續在途的請求;情境 B 目前
  1000 VU + 1–3 秒思考時間 ≈ 500/s,思考時間要壓到 0.3 秒上下才到得了 3000/s。
- 本機測試拓樸撞過 Windows 埠耗盡(investigation run11),加壓前要先確認客戶端撐得住。

**這是需要使用者拍板的取捨**(會改變情境腳本的核心行為與可比性),不是可逆的實作細節。
...
- **3-1(2026-09-23)**:併發 50→1600,ops/s 從 1700 起每翻倍減半,`ops/s × 併發` ≈ 85k–92k 常數,
  throttled 全 0。**拐點在 50 以下,本輪沒量到**;單一 key 在 50 併發以上交不出 3000/s。
  詳見 `docs/load-test-report.md` §14.4。
- **3-2(2026-09-23)**:併發 1–50。**重試風暴坐實**:CAS 嘗試 ÷ 成功寫入 = 1.00 / 1.99 / 4.69 / 8.70 / 11.4 / 16.1 / 25.6,
  在 Redis 與 client 都未飽和的低併發就已等於 N,是樂觀 CAS 的本性。**拐點在單 key 在途 10–20**:
  10 時放行 3000/s、p99 7ms;20 時 p99 40ms;30 起放行掉到 capacity 以下。
  瓶頸端未坐實(Redis 單核 67–69% 未滿,bench 容器 ~110% 持平)。詳見報告 §14.5。
- **3-3(2026-09-23)**:capacity 極大、純 CAS。放行 6,240 / 5,825 / 5,364 / 4,680 / 3,383/s(併發 1/2/5/10/20),
  **上限在併發 1**。Redis 與 Lettuce I/O 執行緒都只到約 69%,不是 CPU 瓶頸,是往返延遲 + 單版本只能一個 CAS 成功。
  詳見報告 §14.6。

**✅ 階段 3 結論**:單 key 安全範圍約**在途 ≤10**(放行 ≤4,700/s、p99 ≤14ms),絕對上限約 6,200/s。
換更大的 Redis 節點無效;要拆 key 或換掉樂觀 CAS。

⚠️ **§5.1 的公式需要改寫(3-2 的發現,待使用者確認)**:限制單 key 的是**在途數**而不是速率,
同一個 Redis 節點上拆 key 就能降低每片的在途數與重試數。分片數比較接近
`ceil(預期在途數 ÷ 單 key 安全在途數)`,而「預期在途數」= 到達率 × 單次檢查耗時。

---

## 5. 階段 4:算出 sharded counter 要拆幾片

### 5.1 計算(2026-09-23 改寫,使用者確認)

原公式 `ceil(目標流量 ÷ 單節點安全上限)` 把上限當成**速率**。階段 3 顯示限制單一 key 的是**在途數**
(同一版本只能有一個 CAS 成功,在途越多重試越多,§4.3),而且瓶頸不在 Redis CPU,
同一個 Redis 節點上拆 key 就有效。改為:

```
預期在途數   = 目標流量 × 單次 global 檢查耗時
至少分片數   = ceil( 預期在途數 ÷ 單 key 安全在途數 )
```

- 單 key 安全在途數 = **10**(報告 §14.6:放行 4,680/s ≥ 3000、p99 14ms;20 時 p99 已 36ms)
- 分片後每片的 capacity = 目標流量 ÷ 片數

### 5.2 ✅ 「目標流量」= 3000/s(2026-09-23 使用者決定)

採設計文件的 `global-capacity=3000`/s。不等階段 2 的下游承載上限,所以階段 2 不是階段 4 的前置。

### 5.3 結論

**單次檢查耗時**取正式站實測的 global 層 p99 **3.5ms**(報告 §14.3,輪 1-2)。它是在約 65 次/秒的低負載下量的,
3000/s 時只會更高,所以把它當**下限**看:

| 單次耗時 | 預期在途數(× 3000/s) | 至少分片數 |
|---|---|---|
| 1ms | 3 | 1 |
| **3.5ms(實測)** | **10.5** | **2** |
| 6.5ms | 19.5 | 2 |
...
**結論:最少 2 片;建議 4 片**(每片 capacity 750/s)。理由:3.5ms 算出的 10.5 只比門檻高一點點,
而負載一上來單次耗時就會變長,且在途數越過拐點後是正回饋(延遲 ↑ → 在途 ↑ → 重試 ↑)。
4 片可以承受單次耗時漲到約 13ms,是 2 的冪、之後好再拆。

**已知限制**:
- 3000/s 下的真實單次耗時沒有量過(階段 1 產不出這個流量)。
- 分片後「全站 3000/s」變成「每片 750/s」,請求在片之間分配不均時,整體放行量會略低於 3000。
- 實作分片會改正式程式碼的限流行為,屬另一個需要確認的決策,本計畫不含實作。
- 另一個方向是改用單一原子操作的 Lua token bucket(沒有 CAS 重試),評估中,見 §6。

---

## 6. 階段 5:評估原子 Lua token bucket(2026-09-25 開始)

### 6.1 要回答的問題

§5.3 最後一條留下的方向:把全域限流從 Bucket4j 樂觀 CAS 換成**單一原子 Lua 腳本**,一把 key
能不能直接撐住 3000/s,讓 sharded counter 變成不需要?

階段 3 的結論是「瓶頸不是 Redis CPU,是往返延遲 + 同一版本只能一個 CAS 成功」。Lua 腳本在 Redis 內
一次做完讀、算、寫,沒有版本衝突也沒有重試,理論上限改由 **Redis 單執行緒跑腳本的速度**決定。
這個推論要在同一台 OCI 主機上實測才算數。

本階段**只評估,不改正式程式碼**。量完後由使用者決定走 Lua、sharded counter,或兩者都不做。

### 6.2 做法:沿用階段 3 的工具,只換檢查路徑

`ratelimit-bench` 新增 `BENCH_MODE=lua`([`token_bucket.lua`](../../load-test/ratelimit-bench/src/main/resources/token_bucket.lua)),
其餘(拋棄式 Redis、單一共享連線、閉環量法、`sweep.sh` 的指令計數與 CPU 快照)與階段 3 完全相同,
數字可以直接和報告 §14.5、§14.6 對照。本機冒煙已確認放行量精準貼齊 capacity(bench README)。

| 輪 | 指令(在 OCI 主機上,拋棄式 Redis 已 up) | 對照 |
|---|---|---|
| 5-1 | `BENCH_MODE=lua BENCH_REDIS_PASSWORD=... bash /tmp/sweep.sh 1 2 5 10 20 50 100 200` | 3-2(capacity 3000) |
| 5-2 | `BENCH_MODE=lua BENCH_CAPACITY=100000000 BENCH_REDIS_PASSWORD=... bash /tmp/sweep.sh 1 5 20 50 100 200` | 3-3(桶取不空,純寫入上限) |

每輪約 `階數 × 35 秒`;打的是拋棄式 Redis,不碰正式 Redis 與正式站流量。上傳與收工步驟同 bench README
「在 OCI 上跑」(jar 要重新建置、`sweep.sh` 要重新上傳)。

### 6.3 判定標準(量之前先定好)

1. **正確性**(5-1):capacity 3000 時,放行/s 在任何一階都落在 3000 ± 1%,不多放也不因競爭掉下去。
   不成立就不必往下看,Lua 版本身有問題。
2. **沒有重試**(5-1、5-2):`cmdstat_evalsha` 的 `calls` ≈ bench 的 ops ×(暖身 + 計時)÷ 計時。
   比值明顯大於 1 表示有預期外的重送。
3. **安全在途數**(5-1):沿用階段 3 的標準,p99 ≤ 14ms 的最大併發即 Lua 的「單 key 安全在途數」。
   代入 §5.1 公式:
   - 安全在途 ≥ 40 → **一把 key 就撐得住**,連單次耗時漲到 13ms(§5.3 選 4 片時的餘裕)都涵蓋,不需要分片
   - 11–39 → 一把 key 撐得住實測耗時,但餘裕不如 4 片 CAS,要比較兩案
   - ≤ 10 → 與 CAS 沒有差別,放棄 Lua 方向
4. **上限在哪一端**(5-2):放行/s 隨併發上升到哪裡停止,以及停止時 Redis CPU 是否接近 100%。
   注意被拒絕的請求一樣會跑一次腳本,所以要看的是**總檢查次數/s**,不是放行數:搶票瞬間的到達率
   可能遠超 3000/s,多出來的全是被拒絕但仍要進 Redis 的檢查。

### 6.4 不在本階段範圍

- 正式實作(要寫 ADR 0010、Testcontainers 併發測試,並決定只換全域層還是四層一起換)
- 腳本內用 Redis `TIME` 與 `HSET` 的持久化成本:拋棄式 Redis 與正式站同樣開 `appendonly yes`,已涵蓋

### 6.5 結果(2026-09-25,報告 §14.7)

| 判定項 | 結果 |
|---|---|
| 1. 正確性 | ✅ 每階放行 89,999–90,002 次 / 30s,精準等於 3000/s |
| 2. 沒有重試 | ✅ `evalsha` = `hset` = ops(含暖身),比值 0.96–0.98 |
| 3. 安全在途數 | ✅ **≥ 200**(量測上限;併發 200 時 p99 5.0ms,未碰到 14ms)→ 落在「≥ 40」 |
| 4. 上限在哪一端 | 約 **8.5 萬次檢查/秒**,Redis CPU 92–95%。上限從 CAS 的版本衝突移到 Redis 單執行緒 |

**結論:一把 key 用原子 Lua token bucket 就撐得住 3000/s,sharded counter 不需要。**
以 §5.1 公式回推:3000/s × 單次耗時要到 67ms 以上,在途數才會超過 200;而量測範圍內單次耗時最高約 5ms(p99)。

已知限制見報告 §14.7(拋棄式 Redis 只有這一把 key 的流量;正式 Redis 是共用的)。

**下一步(會改正式站行為,待使用者決定)**:要不要把 `RateLimiterService` 的全域層換成 Lua 腳本,
以及其他三層(user / ip / token_user)要不要一起換。要寫 ADR 0010 與 Testcontainers 併發測試。
